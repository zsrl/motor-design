...
12、Torque Angle (degree)（功角）需要在25-40度之间，如果仿真结果显示功角过大，可能导致电机运行不稳定和振动增加，此时需要调整磁极类型、槽型或增加斜槽等参数来降低功角；反之，如果功角过小，则可以适当调整磁极类型或槽型来提高功角。如果功角较小，通过减定子和转子的高度、增加绕线圈数，来调节，反之亦然。
